...
export function useFinanceStore(userId: string) {
  ...
  const apiAvailable = useRef(!isGuest);
  ...
  const deleteTransaction = useCallback((id: string) => {
    ...
    if (apiAvailable.current) {
      fetch(`/api/transactions?userId=${userId}&id=${id}`, { method: 'DELETE' }).catch(console.error);
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [userId]);
  ...
  const seedDemoData = useCallback(async () => {
    ...
    try {
      if (apiAvailable.current) {
        await fetch(`/api/seed?userId=${userId}`, { method: 'POST' });
        const res = await fetch(`/api/transactions?userId=${userId}`);
        const data = await res.json();
        setTransactions(data.transactions || []);
      } else {
  ...
  const clearAllData = useCallback(async () => {
    ...
    if (apiAvailable.current) {
      try {
        await fetch(`/api/clear?userId=${userId}`, { method: 'POST' });
      } catch { /* ignore */ }
    } else {
